...
class TaxiViewsManagerActor(val conn: AsterixConnection,
                               override val sourceActor: ActorRef
                              )(implicit ec: ExecutionContext) extends ViewsManagerActor(TaxiDataStoreActor.Name, sourceActor) {

  import TaxiViewsManagerActor._
  ...
  override def createViewActor(key: String, query: DBQuery, fView: Future[ViewMetaRecord]): ActorRef = {
    query.predicates.find(_.isInstanceOf[KeywordPredicate]).map(_.asInstanceOf[KeywordPredicate]) match {
      ...
      case None =>
        context.actorOf(Props(classOf[TaxiSummaryView],
          conn, summaryViewTemplate(TaxiSummaryView.SummaryLevel), sourceActor, fView, ec),
          key)
    }
  }
...
object TaxiViewsManagerActor {

  val ViewMetaDataSetName = "tripdata.viewMeta"
  val ViewMetaCreateAQL =
    s"""
       |use dataverse ${TaxiDataStoreActor.DataVerse}
